...
//                                		student_part = self.pos.db.get_partner_by_ean13(user_barcode);
                                		...
                                		_.each(match_students, function(stud){
                                			if (stud.name == user_barcode.split(' ')[0]) {
                                				students.push(stud);
                                			}
    					                });
                                		selecteduser = students[0];
...
//                                		student_part = self.pos.db.get_partner_by_ean13(user_barcode);
				                        ...
				                        {text: _t("Ok"), click: function() {
				                            ...
				                            if(!selecteduser.pin || selecteduser.pin == ''){
				                            	...
				                            	if(select_user == 'Staff') {
						                                    ...
						                                    function(faclt_res) {
						                                    	if (faclt_res && faclt_res[0]) {
							                                    	if(faclt_res[0].available_balance > 0 ){
					                                                    ...
					                                                    selectedOrder.set_user_balance(faclt_res[0].emp_balance_amount);
				                            		...
				                            		new instance.web.Model("op.student").get_func("search_read")([['id', '=', selecteduser.id]]).pipe(
						                                    function(stud_res) {
						                                    	if (stud_res && stud_res[0]) {
							                                    	if(stud_res[0].stud_balance_amount > 0 ){
							                                    		selectedOrder.set_user_mode('student');
							                                    		selectedOrder.set_customer_name(selecteduser);
					                                                    selectedOrder.set_user_balance(stud_res[0].stud_balance_amount);
					                                                    selectedOrder.set_studentId(selecteduser.id);
					                                                    selectedOrder.set_facultyId(false);
					                                                    $('#select_user').hide();
					                                                    $('#logout').show();
					                                                    $('#selected_user').html('Welcome ' + selecteduser.name +' !')
					                                                    $('#user_balance').html('Your balance is ' + stud_res[0].stud_balance_amount.toFixed(2))
					                                                } else {
					                                                    alert ('You have insufficient balance.. !');
					                                                    $('#selected_user').html('')
					                                                    $('#user_balance').html('')
                ...
                $("#search_by").change(function() {
                	if($("#search_by").val() == 'Barcode'){
                		$("#user_label").html("Enter Barcode")
                	} else {
...
//                interface: this.get_interface(),
        	...
        	if (this.editable && order.getSelectedLine()) {
                ...
                if( mode === 'quantity'){
                    ...
                    if (order.get_facultyId()) {
                    	if (order.getTotalTaxIncluded() > order.get_customer_name().emp_balance_amount){
                        	alert ('Sorry you have insufficient balance.. !');
    ...
    instance.point_of_sale.PosModel = instance.point_of_sale.PosModel.extend({
        models: [
        {
            ...
            model:  'op.student',
            fields: ['name','ean13','pin','middle_name','last_name'],
            domain: null,
            loaded: function(self,op_students){ self.op_students = op_students;},
        },{
            model:  'hr.employee',
        ...
        },
        {
            ...
            fields: ['display_name', 'list_price','price','pos_categ_id', 'taxes_id', 'ean13', 'default_code', 
                     ...
                     'product_tmpl_id','qty_available','type'],
            domain: function(self){ 
            	if(self.shop.category_ids && self.shop.category_ids.length > 0){
            		return [['sale_ok','=',true],['available_in_pos','=',true], ['pos_categ_id', 'child_of', self.shop.category_ids]];
            	}else{
            		return [['sale_ok','=',true],['available_in_pos','=',true]];
            	}
            },
            context: function(self){ return { pricelist: self.pricelist.id, display_default_code: false, location: self.config.stock_location_id[0]}; },
